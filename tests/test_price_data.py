...
def test_combine_deduplicates_and_later_frame_wins():
    older = pd.DataFrame({
        "Date": ["2026-09-19"], "Commodity": ["Urea"], "Price": [440.0],
        "Unit": ["USD/T"], "Source": ["older"],
    })
    newer = pd.DataFrame({
        "Date": ["2026-09-19"], "Commodity": ["Urea"], "Price": [450.25],
        "Unit": ["USD/T"], "Source": ["newer"],
    })
...
def test_load_prices_falls_back_when_live_sheet_has_bad_schema(monkeypatch, tmp_path: Path):
    backup = pd.DataFrame({
        "Date": ["2026-09-19"], "Commodity": ["Sulfur"], "Price": [3000.0],
        "Unit": ["CNY/T"], "Source": ["backup"],
    })
